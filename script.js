...
if (searchInput && searchResults) {
  searchInput.addEventListener('input', () => {
    ...
    const elements = document.querySelectorAll('p, h2, h3, li, .blog-card, .faq-item, .price-card');
    const matches = [];
    elements.forEach(el => {
    ...
    snapshot.forEach(doc => {
      ...
      container.innerHTML += `
        <div class="blog-card">
          ...
          <a href="blog-post.html?id=${doc.id}" style="color:var(--green); font-weight:bold;">Read More →</a>
        </div>
      `;
    });
...
if (document.getElementById('videosContainer')) loadVideos();
...
// Note: The patient record search and render logic is kept inside patient-record.html
// because it uses page-specific DOM elements, but the helper functions above are available globally.
